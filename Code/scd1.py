...
class SCD1:
    ...
    def reset(self):
        ...
        if reset_input.lower() != 'yes':
            ...
            for file_ in os.listdir(d.dest_path_scd1):
                if file_.startswith('Customer_Incr') or file_.startswith('Provider_Incr'):
                    shutil.move(os.path.join(d.dest_path_scd1, file_), d.incr_folder_path)
                elif file_.startswith('Customer') or file_.startswith('Provider'):
                    shutil.move(os.path.join(d.dest_path_scd1, file_), d.fr_folder_path)
    ...
    def lndng_to_stgng(self):
        query = '''
                ...
                Cust_PH_NUm,create_ts,load_ctl_key,Prov_id,Prov_name,Prov_address,Cust_key,checksum,trans_cd)
                ...
                FROM
                (Select distinct C.Cust_id, C.Cust_name, C.Cust_Address, C.Cust_bn_cd, C.Cust_SSN, C.Cust_cvrg_EFFT_date, C.Cust_cvrg_term_date,
                ...
                LEFT JOIN target_eligibility t ON t.cust_id = s.cust_id
                '''
        self.cursor.execute(query)
        self.cursor.execute("delete e.* from target_eligibility e join staging s on e.cust_key = s.cust_key where s.trans_cd = 'U'")
        self.connection.commit()
    ...
    def incr_lndng_to_stgng(self):
        query = '''
                ...
                Cust_PH_NUM,Prov_id,Prov_name,Prov_address,load_ctl_key,create_ts,Action_Indicator)
                Select distinct C.Cust_id, C.Cust_name, C.Cust_Address, C.Cust_bn_cd, C.Cust_SSN, C.Cust_cvrg_EFFT_date, C.Cust_cvrg_term_date,
                C.Cust_Email, C.Cust_PH_NUm,P.Prov_id, P.Prov_name, P.Prov_address,c.load_ctl_key, c.create_ts,c.Action_Indicator
        ...
        self.cursor.execute(query)
        self.cursor.execute(
            "delete e.* from incr_eligibility e join incr_staging s on e.cust_id = s.cust_id where s.Action_Indicator = 'U'")
        self.connection.commit()
    ...
    def stgng_to_base(self):
        query = '''
                ...
                '''
        self.cursor.execute(query)
        self.cursor.execute("Delete from TARGET_ELIGIBILITY where cust_id not in (select cust_id from staging)")
        self.cursor.execute(
    ...
    def incr_stgng_to_base(self):
        query = '''
                    ...
                    Cust_PH_NUm,Prov_id,Prov_name,Prov_address,create_ts,load_Ctl_key)
                    Select Cust_id, Cust_name, Cust_Address, Cust_bn_cd, Cust_SSN, Cust_cvrg_EFFT_date, Cust_cvrg_term_date,
                    Cust_Email, Cust_PH_NUm, Prov_id, Prov_name, Prov_address, create_ts, load_Ctl_key
                    FROM incr_staging;
                '''
        self.cursor.execute(query)
        self.cursor.execute("delete e.* from incr_eligibility e join incr_staging s on e.cust_id = s.cust_id where s.Action_Indicator = 'D'")
        self.cursor.execute(
    ...
    def insert_customer(self, current_file_index, cust_files, date_uuid, check):
        if current_file_index < len(cust_files):
            ...
            if file not in check:
                ...
                shutil.move(cust_file_path, d.dest_path_scd1)
                self.connection.commit()
                if self.process_all.lower() != 'yes':
    ...
    def insert_provider(self, current_file_index, prov_files, date_uuid, check):
        if current_file_index < len(prov_files):
            ...
            if file not in check:
                ...
                shutil.move(prov_file_path, d.dest_path_scd1)
                self.connection.commit()
                if self.process_all.lower() != 'yes':
                    print(f"Records from {file} inserted and moved to Archives folder successfully!")
            else:
    ...
    def execution(self, select_folder, keyword, user_input):
        ...
        check_existing = os.listdir(d.dest_path_scd1)
